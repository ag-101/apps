module ApplicationHelper
  ...
  def breadcrumbs
    ...
    add_breadcrumb_item(t('text.apps'), root_path) if check_role 'admin'
    add_breadcrumb_item(@app.name, app_path(@app)) if @app
    
    add_breadcrumb_item(t('text.forms'), app_forms_path) if params[:controller] == 'apps/forms'
    add_breadcrumb_item(@apps_construct.name, app_form_path(@app, @apps_construct)) if @apps_construct and !@submissions
    ...
    add_breadcrumb_item(@workflow.name, app_workflow_path(@app, @workflow)) if @workflow  


    content_tag :ol, :class=>'breadcrumb' do
      @breadcrumbs.each_with_index do |(k,v), index|
        concat (content_tag :li do
          finished = (index+1 == @breadcrumbs.size) ? link_to(k, '#', :class=>'btn btn-xs current_page' ) : link_to(k, v[:url], :class=>'btn btn-xs btn-default')
        end)
      end
    end
  end
        ...
        if (@app and @app.id)
          ...
          if check_role 'admin', @app.id
            content += content_tag :li do
               ...
               nested_content += nav_link t('text.forms'), app_forms_path(@app), 'apps/forms', {:class=>'admin_link'}
               nested_content += nav_link t('text.responses'), app_submissions_select_path(@app), 'apps/submissions', {:class=>'admin_link'}          
               nested_content += nav_link t('text.permissions'), role_path(@app.id), 'roles', {:class=>'admin_link'}
               nested_content += nav_link t('text.workflows'), app_workflows_path(@app), 'apps/workflows', {:class=>'admin_link'} if @app.app_type == 2
               nested_content
            end         
          end
